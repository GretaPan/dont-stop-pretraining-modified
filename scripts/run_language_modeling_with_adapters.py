...
def main():
    ...
    if len(model.config.adapters.adapters) == 0:  # no adapter loaded
        ...
        # Convert PfeifferConfig to AdapterConfig
        wrap_config = AdapterConfig(
                mh_adapter=config.mh_adapter,
                output_adapter=config.output_adapter,
                reduction_factor=config.reduction_factor,
                non_linearity=config.non_linearity,
                original_ln_before=config.original_ln_before,
                original_ln_after=config.original_ln_after,
                ln_before=config.ln_before,
                ln_after=config.ln_after,
                init_weights=config.init_weights,
                is_parallel=config.is_parallel,
                scaling=config.scaling,
                use_gating=config.use_gating,
                residual_before_ln=config.residual_before_ln,
                adapter_residual_before_ln=config.adapter_residual_before_ln,
                inv_adapter=config.inv_adapter,
                inv_adapter_reduction_factor=config.inv_adapter_reduction_factor,
                cross_adapter=config.cross_adapter,
                leave_out=config.leave_out,
                phm_layer=config.phm_layer,
                phm_dim=config.phm_dim,
                factorized_phm_W=config.factorized_phm_W,
                shared_W_phm=config.shared_W_phm,
                shared_phm_rule=config.shared_phm_rule,
                factorized_phm_rule=config.factorized_phm_rule,
                phm_c_init=config.phm_c_init,
                phm_init_range=config.phm_init_range,
                learn_phm=config.learn_phm,
                hypercomplex_nonlinearity=config.hypercomplex_nonlinearity,
                phm_rank=config.phm_rank,
                phm_bias=config.phm_bias
            )

        model.add_adapter(args.adapter_name, config=wrap_config) 
